...
type Routable interface {
	Route(method, path string, hs ...HandleFunc)
}
...
type MapBasedHandler struct {
	routes            sync.Map
	globalMiddlewares []HandleFunc
}

func NewMapBasedHandler(middlewares ...HandleFunc) *MapBasedHandler {
	wares := make([]HandleFunc, 0, len(middlewares))
...
// Route 实现Router接口
func (h *MapBasedHandler) Route(method, path string, handlers ...HandleFunc) {
	k := h.key(method, path)
	hs := make([]HandleFunc, 0, len(handlers))
...
func (h *MapBasedHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	handlers := hs.([]HandleFunc)
	ctx.hs = h.globalMiddlewares
	ctx.hs = append(ctx.hs, handlers...)
}
...
func NewTreeBasedHandler(middlewares ...HandleFunc) *TreeBasedHandler {
	wares := make([]HandleFunc, 0, len(middlewares))
	wares = append(wares, middlewares...)
...
func (h *TreeBasedHandler) Route(method, path string, handlers ...HandleFunc) {
	cur := h.root
...
// 在root下建子树
func (h *TreeBasedHandler) createSubTree(root *Node, method string, path []string, handlers ...HandleFunc) {
	cur := root
	for _, p := range path {
		node := NewNode(p)
...
func (h *TreeBasedHandler) Query(root *Node, method string, path string) []HandleFunc {
	paths := strings.Split(strings.Trim(path, "/"), "/")
	cur := root
	for _, p := range paths {
		log.Printf("p:%s\n", p)
